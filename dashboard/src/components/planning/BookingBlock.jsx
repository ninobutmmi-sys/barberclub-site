  ...
  return (
    <>
      ...
      >
        ...
        {isOnline && (
          <div className="planning-block-source" title="Réservé en ligne" style={noShowCount > 0 ? { right: 22 } : undefined}>
        ...
        {/* No-show badge — top right corner */}
        {noShowCount > 0 && (
          <div className={`planning-block-noshow-indicator${noShowCount >= 2 ? ' recidiviste' : ''}`} title={`${noShowCount} faux plan${noShowCount > 1 ? 's' : ''} non payé${noShowCount > 1 ? 's' : ''}`}>
            {noShowCount}
          </div>
        )}
