    ...
    <>
      ...
      {isMobile ? (
        <div className="page-header" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 4, padding: '8px 12px' }}>
          {/* Row 1: Month nav + KPIs + Bloquer */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <button className="plan-nav-btn" onClick={goPrevMonth} style={{ width: 24, height: 24 }}><ChevronLeft /></button>
              <span style={{ fontSize: 13, fontWeight: 700, textTransform: 'capitalize', minWidth: 80, textAlign: 'center' }}>{monthDisplay}</span>
          ...
          {nextBooking && format(currentDate, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', marginTop: 2, background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: 8 }}>
              <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#3b82f6', flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  <span style={{ color: '#3b82f6', fontWeight: 700 }}>{nextBooking.start_time?.slice(0, 5)}</span> {nextBooking.client_first_name} {nextBooking.client_last_name} · {nextBooking.service_name}
                </div>
              </div>
              {nextBooking.client_phone && (
                <a href={`tel:${nextBooking.client_phone.replace(/\s/g, '')}`} style={{ width: 30, height: 30, borderRadius: 7, background: 'rgba(34,197,94,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#22c55e', textDecoration: 'none', flexShrink: 0, border: '1px solid rgba(34,197,94,0.2)' }}>
